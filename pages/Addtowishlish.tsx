...
function Addtowishlish() {
    ...
    const [wishlist, setWihlist] = useState([]);
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <div  key={item.id} className="flex flex-row drop-shadow-lg rounded-lg bg-white p-2 w-80 mt-5  h-50 pr-[20px] ml-7 ">
        <img
        ...
        <button  onClick={()  => handleDelete(item.id)} className="drop-shadow-lg rounded-lg bg-white text-red-800 text-xs w-12 h-5 absolute top-14 left-36">Delete</button>
        </div>
